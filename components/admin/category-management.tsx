...
export function CategoryManagement() {
  ...
  const handleSaveCategory = () => {
    if (!categoryForm.name || !categoryForm.nameVi) {
      toast({
        title: t("error"),
    ...
    // In a real app, this would save the category via API
    console.log("Saving category:", categoryForm)
    toast({
      title: t("success"),
      description: editingCategory ? t("categoryUpdated") : t("categoryAdded"),
    })
  ...
  const handleDeleteCategory = (categoryId: string) => {
    ...
    if (category && category.productCount > 0) {
      toast({
        title: t("error"),
        description: t("cannotDeleteCategoryWithProducts"),
        variant: "destructive",
      })
      return
    }
  ...
  const handleToggleStatus = (categoryId: string, isActive: boolean) => {
    // In a real app, this would update the category status via API
    console.log(`Toggling category ${categoryId} status to:`, isActive)
    toast({
      title: t("success"),
      description: isActive ? t("categoryActivated") : t("categoryDeactivated"),
    })
  }
  ...
  return (
    <div className="space-y-6">
          ...
          <CardContent className="p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-2xl font-bold">{mockCategories.filter((c) => c.isActive).length}</p>
              </div>
            </div>
          </CardContent>
        </Card>
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-2xl font-bold">
                  {Math.round(mockCategories.reduce((sum, c) => sum + c.productCount, 0) / mockCategories.length)}
                </p>
              </div>
            </div>
          </CardContent>
      ...
      <Card>
        <CardHeader>
          <CardTitle>
            {t("categories")} ({filteredCategories.length})
          </CardTitle>
        </CardHeader>
        <CardContent>
          <Table>
              ...
              {filteredCategories
                ...
                .map((category) => (
                  <TableRow key={category.id}>
                    ...
                    <TableCell className="text-sm text-muted-foreground">{formatDate(category.updatedAt)}</TableCell>
                    <TableCell>
                      <DropdownMenu>
          ...
          {filteredCategories.length === 0 && (
            <div className="text-center py-8 text-muted-foreground">{t("noCategoriesFound")}</div>
          )}
        </CardContent>
      </Card>
        ...
        <DialogContent className="max-w-2xl max-h-[90vh] overflow-y-auto">
          ...
          <div className="space-y-6">
            ...
            <div className="flex gap-2 pt-4">
              <Button variant="outline" className="flex-1 bg-transparent" onClick={() => setShowCategoryModal(false)}>
                {t("cancel")}
              </Button>
              <Button className="flex-1" onClick={handleSaveCategory}>
                {editingCategory ? `${t("save")} ${t("changes")}` : `${t("add")} ${t("category")}`}
              </Button>
            </div>
          </div>
        </DialogContent>
